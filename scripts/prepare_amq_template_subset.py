...
def _template_runtime_repair_advice(errors: list[str]) -> str:
    ...
    if any(
        ...
        advice.append(
            "隔离运行触发联网下载：先区分用户数据与算法必需资产。若下载的是"
            " GeoJSON/CSV/图像等用户数据，而真实算法支持内存对象或文件对象，应把"
            "公开接口从 dataset_name/URL/服务器路径重构为 JSON 内容、文本或 Base64，"
            "在 main_process 内写入临时文件或直接构造领域对象；契约使用仓库示例"
            "结构的最小内联数据。若下载的是预训练 checkpoint/权重等算法必需资产，"
            "只允许使用仓库已提交或已声明随部署提供的真实文件，并从仓库相对路径"
            "加载。不要关闭无网络验证、不要 mock 下载，也不能用随机权重替代预训练"
            "资产；仓库没有必需算法资产时应明确拒绝该能力。"
        )
