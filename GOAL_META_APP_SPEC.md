...
| 用户确认与规格整理 | **补充约束、确认记录**、产物样例 | ArtifactSpec 样例✅；补充约束与确认记录无 | ❌ 缺 |
...
| 运行与交付 | 策略/异常入口/预览/确认记录 | strategy✅、状态机异常态✅；确认记录❌ |
...
| trace → ArtifactSpec 编译 | ✅ 代码完成，**未 commit** |
...
| **P1** | 想定解析结构化；状态断言；服务契约；用户确认与补充约束 |
